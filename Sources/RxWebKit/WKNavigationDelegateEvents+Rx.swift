...
public extension Reactive where Base: WKWebView {
  ...
  typealias WKNavigationEvent = (webView: WKWebView, navigation: WKNavigation)

  /// WKNavigationFailedEvent emits a tuple that contains both
  /// WKWebView + WKNavigation + Swift.Error
  typealias WKNavigationFailEvent = (webView: WKWebView, navigation: WKNavigation, error: Error)
  ...
  typealias WKNavigationChallengeEvent = (webView: WKWebView, challenge: URLAuthenticationChallenge, handler: ChallengeHandler)

  /// DecisionHandler this is the block exposed to the user on subscription
  typealias DecisionHandler = (WKNavigationResponsePolicy) -> Void
  /// WKNavigationResponsePolicyEvent emits a tuple event of  WKWebView + WKNavigationResponse + DecisionHandler
  typealias WKNavigationResponsePolicyEvent = (webView: WKWebView, reponse: WKNavigationResponse, handler: DecisionHandler)
  /// ActionHandler this is the block exposed to the user on subscription
  typealias ActionHandler = (WKNavigationActionPolicy) -> Void
  /// WKNavigationActionPolicyEvent emits a tuple event of  WKWebView + WKNavigationAction + ActionHandler
  typealias WKNavigationActionPolicyEvent = (webView: WKWebView, action: WKNavigationAction, handler: ActionHandler)
  ...
  @available(iOS 9.0, *)
  var didTerminate: ControlEvent<WKWebView> {
  ...
  var didReceiveServerRedirectForProvisionalNavigation: ControlEvent<WKNavigationEvent> {
    let source: Observable<WKNavigationEvent> = delegate
  ...
  var decidePolicyNavigationResponse: ControlEvent<WKNavigationResponsePolicyEvent> {
  ...
  var decidePolicyNavigationAction: ControlEvent<WKNavigationActionPolicyEvent> {
